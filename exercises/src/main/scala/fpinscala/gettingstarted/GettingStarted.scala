...
object MyModule {
  ...
  def fib(n: Int): Int = {
    ...
    def go(previous: Int, current: Int, i: Int): Int = {
      if (i == 0) previous
      else go(current, (previous + current), i - 1)
    }
    go(0, 1, n)
  }
...
object PolymorphicFunctions {
  ...
  def main(args: Array[String]): Unit = {
    val msg = "IsSorted for %s : %s"
    val arr = Array(1, 2, 3, 4, 5, 6)
    println(msg.format(arr.mkString, isSorted(arr, (a: Int, b: Int) => a > b)))
    val arr1 = Array(10, 2, 3, 4, 5, 6)
    println(
      msg.format(arr1.mkString, isSorted(arr1, (a: Int, b: Int) => a > b))
    )
  }
}
